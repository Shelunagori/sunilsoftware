...
<div  class="maindiv" style="border:solid 1px #c7c7c7;background-color: #FFF;padding: 10px;margin: auto;width:75%;font-size: 12px;">	
	<table width="100%" class="divHeader">
		<tbody><tr>
				<td width="30%"><?php if(!empty(@$journalVoucher->company->logo)){ ?>
				<?php echo $this->Html->image('/img/'.$journalVoucher->company->logo, ['height' => '70px', 'width' => '70px']); ?>
				<?php } ?></td>
				...
				<span style="font-size: 14px;font-weight: bold;"><?=@$journalVoucher->company->name?></span><br/>
				<span><?=@$journalVoucher->company->address?>, <?=@$journalVoucher->company->state->name?></span></br>
				<span> <i class="fa fa-phone" aria-hidden="true"></i>  <?=@$journalVoucher->company->phone_no ?> |  Mobile : <?=@$journalVoucher->company->mobile ?><br> GSTIN NO:
				<?=@$journalVoucher->company->gstin ?></span></td>
			</tr>
			<tr>
				<td colspan="3">
