...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $inward->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $inward->id)]
            )
        ?></li>
...
<div class="inwards form large-9 medium-8 columns content">
    <?= $this->Form->create($inward) ?>
    <fieldset>
        <legend><?= __('Edit Inward') ?></legend>
        <?php
