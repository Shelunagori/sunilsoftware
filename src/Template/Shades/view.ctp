...
/**
  ...
  * @var \App\Model\Entity\Shade $shade
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Shade'), ['action' => 'edit', $shade->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Shade'), ['action' => 'delete', $shade->id], ['confirm' => __('Are you sure you want to delete # {0}?', $shade->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Shades'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Shade'), ['action' => 'add']) ?> </li>
...
<div class="shades view large-9 medium-8 columns content">
    <h3><?= h($shade->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $shade->has('company') ? $this->Html->link($shade->company->name, ['controller' => 'Companies', 'action' => 'view', $shade->company->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($shade->id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Items') ?></h4>
        <?php if (!empty($shade->items)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($shade->items as $items): ?>
            <tr>
                <td><?= h($items->id) ?></td>
                <td><?= h($items->name) ?></td>
